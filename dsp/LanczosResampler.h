...
// This file originally came from the iPlug2 library and has been subsequently
// modified; the following license is copied as required from
...
Copyright (C) the iPlug 2 Developers. Portions copyright other contributors, see
each source file for more information.
 ...
 * @tparam A The Lanczos filter size. A higher value makes the filter closer to
 an ideal stop-band that rejects high-frequency content (anti-aliasing), but at
...
class LanczosResampler {
 private:
  ...
  // know what they're doing ;) (size_t)(8192 * 3 * 192000 / 44100)+1 = 106998
  static constexpr size_t kBufferSize =
      131072;  // Round up to pow2. I don't know why, but it doesn't work
               // otherwise.
 ...
 public:
  ...
  LanczosResampler(float inputRate, float outputRate)
      : mInputSampleRate(inputRate), mOutputSampleRate(outputRate) {
    SetPhases();
    ClearBuffer();

    auto kernel = [](double x) {
      if (std::fabs(x) < 1e-7) return T(1.0);

      return T(A * std::sin(kPi * x) * std::sin(kPi * x / A) /
               (kPi * kPi * x * x));
    };
  ...
  inline size_t GetNumSamplesRequiredFor(size_t nOutputSamples) const {
    /*
     * So (mPhaseIn + mPhaseInIncr * res - mPhaseOut - mPhaseOutIncr *
     * nOutputSamples) * sri > A + 1
     *
     * Use the fact that mPhaseInIncr = mInputSampleRate and find
     * res > (A+1) - (mPhaseIn - mPhaseOut + mPhaseOutIncr * desiredOutputs) *
     * sri
     */
    double res = A + 1.0 -
                 ((double)(mPhaseInNumerator - mPhaseOutNumerator -
                           mPhaseOutIncrNumerator * (long)nOutputSamples)) /
  ...
  inline void PushBlock(T** inputs, size_t nFrames) {
    for (auto s = 0; s < nFrames; s++) {
      for (auto c = 0; c < NCHANS; c++) {
        ...
        mInputBuffer[c][mWritePos + kBufferSize] =
            inputs[c][s];  // this way we can always wrap
      }
  ...
  size_t PopBlock(T** outputs, size_t max) {
    int populated = 0;
    while (populated < max && (mPhaseInNumerator - mPhaseOutNumerator) >
                                  mPhaseDenominator * (A + 1)) {
 ...
 private:
  ...
  void SetPhases() {
    ...
    // honestly I'm fine with that. It's your fault for not using something
    // normal like 44.1k, 48k, or their multiples. (Looking at you,
    // VST3PluginTestHost!)
    auto AssertLongLikeSampleRate = [](double x) {
      if ((double)((long)x) != x) {
        std::cerr << "Expected long-like sample rate; got " << x
                  << " instead! Truncating..." << std::endl;
      }
      return (long)x;
    };
  ...
  // Phase is treated as rational numbers to ensure floating point errors don't
  // accumulate and we stay exactly on. (Issue 15)
...
T LanczosResampler<T, NCHANS, A>::sTable alignas(
    16)[LanczosResampler<T, NCHANS, A>::kTablePoints + 1]
       [LanczosResampler::kFilterWidth];
